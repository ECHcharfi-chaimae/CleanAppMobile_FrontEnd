...
function SendDemand({ route, navigation }) {
  ...
  function AddDemand(e) {
    e.preventDefault();
    axios.post('http://192.168.1.108:8080/api/demandes/add', data).then(res => {
      console.log(res);
    ...
    <ScrollView>
      <View style={styles.container1}>
        <View style={{ flexDirection: "row", marginTop: 50 }} >
          <Img image={route.params.employee.image} />
          <View>
            <Text style={{ ...FONTS.title, marginTop: 10, }}>{route.params.employee.full_name}</Text>
            <Rating imageSize={15} />
