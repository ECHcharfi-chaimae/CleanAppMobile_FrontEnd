...
const Profile = ({ navigation }) => {
	...
	useEffect(() => {
		axios.get(`http://192.168.1.108:8080/api/Employees/${auth.phone}`).then((response) => {
			setEmployee(response.data);
			console.log("chaymae");
		}).catch(err => {
			console.log("errreurdd r" + err);
		})
	}, []);
				...
				<View style={styles.header2}>
					...
					<View style={styles.h}>
						<View style={styles.button} >
							<Botton text="my Articles" bgColor="#2196F3" onPress={() => { navigation.navigate("ListDemandsEmployee") }}></Botton>
