    ...
    const sinUp = () => {
        if (signUp.full_name !== "" && signUp.phone !== "" && signUp.password !== "" && signUp.confirmPass !== "") {
            if (signUp.password === signUp.confirmPass) {
                axios.post("http://192.168.1.108:8080/api/Employees/add", signUp).then(res => {
                    if (res.data === "saved") {
                        navigation.push("SignUpSuccess");
                        storeData('@auth', { "isLogged": true, "phone": login.phone })
                        setAuth({"isLogged" : true, "phone" : signUp.phone });
                    }
                    else if (res.data === "unsaved") {
                        console.log("the phone number exist");
                    }
                }).catch(err => {
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                <View style={{ marginBottom: 10 }}>
                    ...
                    {errors.find(e => e.field === "full_name") !== undefined ? (
                        <Text
                            style={{
                                color: "red",
                                fontSize: 11,
                    ...
                    {errors.find(e => e.field === "phone") !== undefined ? (
                        <Text
                            style={{
                                color: "red",
                                fontSize: 11,
                    ...
                    {errors.find(e => e.field === "password") !== undefined ? (
                        <Text
                            style={{
                                color: "red",
                                fontSize: 11,
